...
from n_transformer_module import *

# 定义Roofline模型中的常数值
theoretical_flop_base = 9465.92  # GFLOP/s 基础时钟理论性能
theoretical_flop_boost = 12714.68  # GFLOP/s 提升时钟理论性能
memory_bandwidth = 360  # GB/s 内存带宽
...
def attention_flop(batch_size, seq_len, embed_size, num_heads):
    # Q, K, V计算
...
# Roofline模型绘制
def draw_roofline(batch_size, seq_len, num_hiddens, num_heads):
    # 计算 Attention FLOPs
    ...
    # 计算性能限制
    performance_base = np.minimum(
        flops, theoretical_flop_base * np.ones_like(flops)
    )  # 基础时钟限制的性能
    performance_boost = np.minimum(
        flops, theoretical_flop_boost * np.ones_like(flops)
    )  # 提升时钟限制的性能
    ...
    plt.legend()
    plt.savefig("./attention_roofline_model.png")
    plt.show()
