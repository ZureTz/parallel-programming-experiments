...
int main(int argc, char **argv) {
  ...
  // Set the tolerance
  tol_digits = 10;
  double tol = 1.0 / pow(10.0, tol_digits + 1.0);

  // Set up the world and get a struct containing all process info needed
  process_data row = set_up_world(Np, N);
  ...
  equation_data equation = random_linear_system(row);
  ...
  double sequential_solution_time, parallel_solution_time;
  parallel_solution_time = MPI_Wtime() - start_time;

  // Gather and stuff
  double *A, *x, *x_star, *x_star_seq, *b;
  if (row.rank == 0) {
  ...
  if (row.rank == 0) {
    ...
    fprintf(file, "%d %d %d %f %f %f\n", Np, N, max_steps,
            sequential_solution_time, parallel_solution_time,
            sequential_solution_time / parallel_solution_time);
    fclose(file);
  }
...
process_data set_up_world(int Np, int N) {
  ...
  row.displ = row.coord * (row.count_min) +
...
equation_data random_linear_system(process_data row) {
  ...
  for (int n = 0; n < row.Np; ++n) {
    ...
    if (n == 0) {
      // Don't use the buffer, just calculate the diagonal block addition
      // in-place
    ...
    if (n < row.Np - 1) {
      if (coord_work != coord_recv) {
        ...
        MPI_Sendrecv(&equation.A[row.displs[rank_recv]], row.counts[rank_recv],
                     row.block_trans_t, rank_recv, n, &(B[0]), B_size,
                     MPI_DOUBLE, rank_send, n, row.comm, MPI_STATUS_IGNORE);
      }
    }
  }
  ...
  // Create temporary x vectors for sending and receiving
  ...
  for (int n = 0; n < row.Np; ++n) {
    ...
    if (n < row.Np - 1) {
      MPI_Recv(x_recv, row.count_max, MPI_DOUBLE, rank_down, 111, row.comm,
               MPI_STATUS_IGNORE);
      MPI_Wait(&send_req, MPI_STATUS_IGNORE);
    }
  }
...
double max_error(double *real_x, double *approx_x, int N) {
  double max = 0;
  for (int i = 0; i < N; ++i) {
    const double error = fabs(real_x[i] - approx_x[i]);
    if (error > max) {
      max = error;
    }
  }
